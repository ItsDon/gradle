...
class DefaultTestClassScannerTest extends Specification {
    ...
    @Test
    void passesPreviousFailedClassFirst() {
        ...
        then:
        1 * processor.processTestClass(_) >> { assert it[0].testClassName == 'Class3' }
        then:
        1 * detector.startDetection(processor)
        then:
        1 * detector.processTestClass(new File("Class1.class"))
    ...
    private FileVisitDetails mockFileVisitDetails(String className) {
        String fileName = "${className}.class"
        return new DefaultFileVisitDetails(new File(fileName), new RelativePath(true, fileName), null, null, null)
    }
}
